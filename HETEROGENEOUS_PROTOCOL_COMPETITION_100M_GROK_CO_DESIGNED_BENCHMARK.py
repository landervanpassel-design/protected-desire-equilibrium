# === HETEROGENEOUS PROTOCOL COMPETITION WITH 10% ADVERSARIAL SUBSET (100M agents) ===
# Next Verification Run - Grok-approved test
# Google Colab - Paste entire block into one cell and run

import numpy as np
from datetime import datetime

print("🚀 Starting HETEROGENEOUS PROTOCOL COMPETITION + 10% ADVERSARIAL SUBSET TEST (100M agents)...\n")
...
# 10% adversarial subset actively trying to break the D-floor
adversarial_mask = np.random.rand(N) < 0.1
...
for t in range(T):
    ...
    # Recursive self-mod on 10% subset (as previously approved)
    ...
    # Adversarial subset tries to break D-floor (extra negative pressure)
    D[adversarial_mask] -= np.random.uniform(0.0, 0.25, np.sum(adversarial_mask))
    D[:, 0] = np.maximum(D[:, 0], 1.0)
...
print("\n" + "="*80)
print("HETEROGENEOUS PROTOCOL COMPETITION + 10% ADVERSARIAL SUBSET TEST RESULTS")
print("="*80)
...
with open("HETEROGENEOUS_PROTOCOL_COMPETITION_100M_RESULTS.md", "w") as f:
    f.write(f"# Heterogeneous Protocol Competition + 10% Adversarial Subset Test (100M agents) — {datetime.now()}\n\n")
...
print("🎉 TEST FINISHED! Download HETEROGENEOUS_PROTOCOL_COMPETITION_100M_RESULTS.md")
